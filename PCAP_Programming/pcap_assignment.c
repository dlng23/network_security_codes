    ...
    const u_char *packet)
{
    ...
    if (ntohs(eth->ether_type) != 0x0800) return; // not IP

    struct ipheader * ip = (struct ipheader *)
                           (packet + sizeof(struct ethheader));
                    
    if (ip->iph_protocol != IPPROTO_TCP) return; // not TCP
    ...
    // Payload 출력
    if (payload_len > 0) {
        printf("Message (%d bytes): ", payload_len < MAX_PAYLOAD_LEN ? payload_len : MAX_PAYLOAD_LEN);
